...
resource "azurerm_virtual_machine" "main" {
  ...
  os_profile {
    ...
    custom_data = file("${path.cwd}/_files/nginx.yaml")
  }
  ...
  provisioner "local-exec" {
    command = "echo ${azurerm_public_ip.stf_public_ip.ip_address} > ./${self.name}.txt"
  }
  ...
  provisioner "file" {
    ...
    connection {
      type = "ssh"
      user = "ubuntu"
      private_key = tls_private_key.stf_ssh_key.private_key_openssh
      host = azurerm_public_ip.stf_public_ip.ip_address
    }
  }
