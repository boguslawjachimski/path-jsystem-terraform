locals {
  ...
  dev = var.environment == "development" ? "nyc1" : ""
  stg = var.environment == "staging" ? "nyc3" : ""
  prod = var.environment != "production" && var.environment !="staging" ? "" : "fra1"
  region = coalesce(local.dev, local.stg, local.prod)
}
...
resource "digitalocean_droplet" "student_droplet" {
  ...
  region = "${var.region}"
  size = var.droplet_size != "" ? var.droplet_size : "s-2vcpu-2gb"
