...
resource "aws_instance" "ec2-eu-west-1" {
  ...
  ami                         = local.instances.eu-west-1.ami
  ...
  tags = {
    Name = "${terraform.workspace}-${local.instances.eu-west-1.instance_name}"
    evn  = local.instances.eu-west-1.enviroment
  }
...
resource "local_file" "host_eu_west_1" {
    content = yamlencode ({
      ansible_host = aws_instance.ec2-eu-west-1.public_ip
      ansible_user = "ubuntu"
      ansible_ssh_private_key_file = "./${terraform.workspace}-key"
    })
    filename = "./inventories/${terraform.workspace}/host_vars/${local.instances.eu-west-1.instance_name}.yaml"
}
